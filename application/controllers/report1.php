...
class Report extends CI_Controller {
	...
	public function index() {
		...
		foreach ($list as $arr=>$row) {
		    $unitcost = $row['inqty'] >0 ? ($row['incost']/(float)$row['inqty']) :0;  //单位成本 
			$inventory1 += $row['qty'];
		...
		$list2  = $this->data_model->get_invoice_info_ini('and billDate<="'.date('Y-m-d').'" group by invId,locationId');   
		foreach ($list1 as $arr=>$row) {
		    $qty = $row['qty']>0 ? -abs($row['qty']) : abs($row['qty']);   //销售在数据库中是负数 在统计的时候应该是正数
			$unitcost = isset($list2['inunitcost'][$row['invId']][$row['locationId']]) ? $list2['inunitcost'][$row['invId']][$row['locationId']] : 0;   //单位成本
	...
	public function puDetail_supplyExporter() {
		...
		$where .= $beginDate ? ' and a.billDate>="'.$beginDate.'"' : ''; 
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : ''; 
		$data['list'] = $this->data_model->get_invoice_info($where.' order by a.buId'); 
	...
	public function salesDetail_inv() {
		...
		if ($profit==1) {
			$info  = $this->data_model->get_invoice_info_ini('and billDate<="'.$endDate.'" group by invId,locationId');   
		}
		foreach ($list as $arr=>$row) {
			...
			$unitPrice = $qty!=0 ? $amount/$qty : 0;                //单位成本
			if ($profit==1) {
				$sum4 += $unitcost = isset($info['inunitcost'][$row['invId']][$row['locationId']]) ? $info['inunitcost'][$row['invId']][$row['locationId']] : 0;   //单位成本
	...
	public function salesDetail_invExporter() {
		...
		if ($profit==1) {
			$data['info']  = $this->data_model->get_invoice_info_ini('and billDate<="'.$endDate.'" group by invId,locationId');   
		}
	...
	public function invBalance() {
		...
		foreach ($list as $arr=>$row) {
		    $unitcost = $row['inqty'] >0 ? ($row['incost']/$row['inqty']) :0;  //单位成本
			$v[$arr]['invNo']         = $row['invNumber'];  
			$v[$arr]['invName']       = $row['invName'];   
			$v[$arr]['spec']          = $row['invSpec']; 
			$v[$arr]['unit']          = $row['mainUnit']; 
	...
	public function deliverSummary() {
		...
		foreach ($list as $arr=>$row) {
		     //期初数量
		    $qty_0      = $row['qty0']; 
			$unitcost0   = $row['inqty0']>0 ? $row['incost0']/$row['inqty0'] :0;  //采购单位成本    
			$cost_0     = $qty_0 * $unitcost0;        //期初成本 = 期初数量*单位成本 
			
		    //结存数量
		    $qty_14    = $row['qty14'];             //结存数量
			$unitcost14   = $row['inqty14']>0 ? $row['incost14']/$row['inqty14'] :0;      
			$cost_14   = $qty_14 * $unitcost14;       //结存成本 =  结存数量*单位成本 

		    for ($i=1;$i<7;$i++) {
			    if ($i==1) {                          //调拨  成本另计算
				    $qty_7  += abs($row['qty1']);   
					$cost_7 += abs($row['qty1']) * $unitcost14; 
				} else {
					$qty_7  += abs($row['qty'.$i]);   
			...
			for ($i=8;$i<13;$i++) {
				if ($i==10 || $i==11 || $i==12 || $i==8) {       //销售、盘亏、其他出库  成本另计算
					...
					$cost_13 += abs($row['qty'.$i]) * $unitcost14;  
				} else {
					$qty_13  += abs($row['qty'.$i]);   
			...
			for ($i=0; $i<15; $i++) {
			    if ($i==0) {
				    ...
				    if ($i==10 || $i==11 || $i==12 || $i==1 || $i==8) {  //销售、盘亏、其他出库、入库调拨、出库调拨  成本另计算
					    ...
					    $v[$arr]['cost_'.$i]  = str_money(abs($row['qty'.$i]) *$unitcost14,2);
	...
	public function oriDetail_detail() {
		...
		foreach ($list as $arr=>$row) {
			...
			if ($row['transType']==153401) {
				...
				$payment2 += $v[$arr]['amountOut']      = $row['payment']>0 ? -$row['payment'] : $row['payment'];   //支出
			}
		}
		$data['data']['rows']      = $v;
		$data['data']['userdata']['date']      = '';
