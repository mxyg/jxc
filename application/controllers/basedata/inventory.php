...
class Inventory extends CI_Controller {
	...
	private function validform($data) {
	    ...
	    !isset($data['name']) || strlen($data['name']) < 1 && str_alert(-1,'商品名称不能为空');
		!isset($data['number']) || strlen($data['number']) < 1 && str_alert(-1,'商品编号不能为空');
		!isset($data['categoryId']) || strlen($data['categoryId']) < 1 && str_alert(-1,'商品类别不能为空');
		!isset($data['baseUnitId']) || strlen($data['baseUnitId']) < 1 && str_alert(-1,'计量单位不能为空');
