...
class Assist extends CI_Controller {
	...
	public function index(){
		...
		$where = $type ? ' and typenumber="'.$type.'"' : ' and 1<>1';
	...
	public  function delete() {
		...
		$data = $this->mysql_model->get_row(CATEGORY,'(id='.$id.') and typeNumber="'.$type.'"'); 
		if (count($data)>0) {
		    $info['isDelete'] = 1;
		    $this->mysql_model->get_count(GOODS,'(categoryId='.$id.')')>0 && str_alert(-1,'辅助资料已经被使用');
			$this->mysql_model->get_count(CONTACT,'(cCategory ='.$id.')')>0 && str_alert(-1,'辅助资料已经被使用');
			$this->mysql_model->get_count(CATEGORY,'(1=1) and find_in_set('.$id.',path)')>1 && str_alert(-1,'不能删除，请先删除子分类！'); 
	...
	private function validform($data) {
		...
		!isset($data['name']) || strlen($data['name']) < 1 && str_alert(-1,'类别名称不能为空');
		!isset($data['typeNumber']) || strlen($data['typeNumber']) < 1 && str_alert(-1,'参数错误');
		if (isset($data['id'])) {
