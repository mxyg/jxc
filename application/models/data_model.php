...
class Data_model extends CI_Model{
	...
	public function get_invoice_info_ini($where='',$type=2) {
		...
		$list = $this->mysql_model->query(INVOICE_INFO,$sql,2);
		foreach($list as $arr=>$row){
		    $v['qty'][$row['invId']][$row['locationId']]   = $row['qty'];              //结存数量（时间段） 
		    $v['inqty'][$row['invId']][$row['locationId']]  = $row['inqty'];   //期初入库数量
			$v['incost'][$row['invId']][$row['locationId']] = $row['incost'];  //期初入库成本
			$v['inunitcost'][$row['invId']][$row['locationId']] = $row['inqty']>0 ? $row['incost']/$row['inqty'] :0;  //期初单位成本	
		}		
		return $v;		
	}	
